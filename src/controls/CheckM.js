...
class CheckM extends React.Component {
  ...
  takeMultiPhoto = () =>{
    ImagePicker.launchCamera(optionsPhotos, (response) => {
      if (response.didCancel) {}
          ...
          this.setState({img:file}); // Variable auxiliar para cargar el view de las fotos 
        }
    });
  }
          ...
          for (var i = 0; i < opciones.length; i++) {
            if (opciones[i].codigo === item.codigo) {
              if (opciones[i].codigo === "si") {
                this.setState({ ifCumple: true });
                this.setState({ ifNocumple: false });
              } else if (opciones[i].codigo === "no") {
                this.setState({ ifNocumple: true });
                this.setState({ ifCumple: false });
              }
        ...
        <Row>
          ...
          <Col>
            ...
            {(this.state.toolbar === true && this.props.check_activo === this.props.id && active === true) ? (
              ...
              : (
                <Icon name='plus' type='font-awesome' color='gray' size={38} resizeMode="contain" onPress={() => {
                  Alert.alert('Seleccione Primero', 'Primero debe calificar una pregunta para hacer una Observación');
                }} />
              )
            }
          </Col>
            ...
            {this.state.ifCumple === false ?
              <View>
                <View style={{ marginTop: '3%' }}>
                  ...
                  <Label style={{ marginTop: 10, fontSize: 18, color: this.props.theme.primary, textAlign: 'center' }}>Fecha de Plazo</Label>
                </View>
                <View>
                  <DatePicker
                    showIcon={false}
...
export default connect(mapStateToProps, mapDispatchToProps)(CheckM);