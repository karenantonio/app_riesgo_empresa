...
import {Input as Input2,Item,Label,Text,Button,Textarea,Content, Body, Icon as IconNB} from 'native-base'
import {View,Image,Alert, ImageBackground, Modal, ScrollView, TouchableOpacity, PermissionsAndroid} from 'react-native'
...
class Check extends React.Component {
  ...
  takeMultiPhoto = () =>{
    ImagePicker.launchCamera(optionsPhotos, (response) => {
      if (response.didCancel) {}
        else if (response.error) {Alert.alert('ImagePicker Error: ', response.error);}
        else if (response.customButton) {}
        else {
          const file = {file:"data:" + response.type + ";base64," + response.data};
  ...
  searchMultiPhoto = () =>{
    ...
    ImagePicker.launchImageLibrary(optionsPhotos, (response) => {
      if (response.didCancel) {}
        else if (response.error) {Alert.alert('ImagePicker Error: ', response.error);}
        else if (response.customButton) {}
        else {
  ...
  componentWillMount()
  {
    ...
    let   fecha       = JSON.parse(this.props.valores.filter(row=>row.Nombre==="fecha")[0].Valor)
    ...
    //FECHA POR DEFECTO AL DIA SIGUIENTE (INDICACION DE FABIAN CARO)
    const manana = new Date((new Date()).getTime() + 24*60*60*1000);
    if(fecha === "")
    {
      fecha = manana.getFullYear() + "-" + manana.getMonth()-1 + "-" + manana.getDate();
    }
  ...
  _renderOptions = (active) => {
    ...
    { 
      ...
      {
        if(this.props.check_default_values[ii].action === "mark" && this.props.check_default_values[ii].id === this.props.id && this.props.check_default_values[ii].time>this.state.presionado.time)
        {
    ...
    return this.state.opciones.map((item,i)=>{
      ...
      return(
        <Col style={{flex:1}} key={i} onPress={()=>{
          if(active===false){
            return;
          }
          ...
          for(var i=0;i<opciones.length;i++)
          {
            if(opciones[i].codigo === item.codigo)
            {
              ...
              this.setState({toolbar:true,presionado:{codigo:item.codigo,time:(new Date()).getTime()}});
            }
          }
          }}>
  ...
  render() {
    ...
    return (<View> 
      <Grid style={{borderRadius:10,margin:5,padding:10,backgroundColor:"transparent"}} onPress={()=>{
        ...
        {/* Muestra Label con cantidad de observaciones sobre cada pregunta */}
        ...
        {/* Lista las preguntas del check */}
        <Row>
          <Col style={{flex:6, borderColor:"grey",borderWidth:1, borderRadius: 20}}>
            <Text style={{fontSize:16, height: '100%', margin: 10, color: this.props.theme.primary}}>{this.props.orden}.{this.state.pregunta}</Text>
          </Col>
        </Row>
        ...
        {/* Caja con botón para ingresar una nueva observación */}
        {(this.state.toolbar === true && this.props.check_activo === this.props.id && active === true) &&
          <View style={{borderWidth: 1, borderRadius: 20, padding: 10, borderColor: 'gray', marginTop:10}}>
        ...
        <ScrollView keyboardShouldPersistTaps={'handled'}>
          <View style={{marginTop: 10,paddingLeft:10,paddingRight:10, alignItems: 'center'}}>
            ...
            {this.state.adjunto !== "" ?
              ...
              {//this.state.fotos.map((foto, i) => Multi imagen
                <ImageBackground resizeMode="contain" style={{height:170,width:230,marginTop:10}} source={{uri:this.state.adjunto.file, isStatic:true}}>
                  <Row>
                    <Col></Col>
                    <Col>
                      <Button style={{backgroundColor: '#c90000', marginLeft: 16,borderRadius:30, width: 47, height: 47}} block onPress={()=>{
                        Alert.alert('Eliminar','¿Segura que desea eliminar esta foto?',
                        [{text: 'Cancelar',onPress: () => {},style: 'cancel'},{text: 'OK', onPress: () => {
              ...
              */}                        
              ...
              <Item regular style={{borderColor:"transparent",marginTop:10}}>
                <Button style={{backgroundColor:this.props.theme.tertiary,width:70}} small onPress={()=>{this.takePhoto();}}>
                  <Body>
                    <Icon name='camera' type="font-awesome" color={"#FFFFFF"}/>
                  </Body>
                  <Text>+</Text>
                ...
                <Text>  o</Text>
                <Button small style={{backgroundColor:this.props.theme.tertiary,width:70,marginLeft:10}}onPress={()=>{this.searchPhoto()}}>
                  <Body>
                    <Icon name='paperclip' type="font-awesome" color={"#FFFFFF"} />
                  </Body>
                  <Text>+</Text>
                ...
                if(this.state.id!==""){
                  if(this.state.comentario!==""){
                    for(var i=0;i<observaciones.length;i++){
                      if(observaciones[i].id === this.state.id){
                        ...
                        //observaciones[i].fotos = this.state.fotos; Multi imagen                      
                      }  
                    }  
                  ...
                  if(this.state.comentario!==""){
                    observaciones.push({
                      ...
                      //fotos:this.state.fotos Multi imagen           
                    });         
                    this.setState({observaciones,modal:false});   
                  }else{
                    Alert.alert('Falta información', 'Para guardar debe ingresar una observación.');
